...
class DataTransformation():
    ...
    def convert_to_currency(self, cell, currency_input, year, currency_required='USD'):
        ...
        cell = (converter.convert(cell, currency_input, currency_required, date=date(year, 1, 1)))
        ...
        try:
            ...
            for currency in currencies_signs:
                if currency in str(x):
                    print(x)
                    x = self.replace_symbol_of_cell(cell=x, symbol_to_be_replaced=currency, replacement='')
                    print(x)
                    x = float(x)
                    x = self.convert_to_currency(cell=x, currency=currencies[currencies_signs.index(currency)],year = date(year, 1, 1))
                else:
                    continue
            return x
        except:
    ...
    def budget_column(self):
        ...
        for element in garbage:
            self.clear_column(column='Бюджет', replacement='', to_be_replaced=element)   
        self.df_cleared['Бюджет'] = self.df_cleared['Бюджет'].apply(lambda y: self._budget(y))
    ...
    def views_column(self):
        self.df_cleared['Зрители'] = self.df_cleared['Зрители'].apply(lambda x: self.convert_to_float(x))
    ...
    def fees_columns(self):
        ...
        for column in columns:
            for element in garbage:
                self.clear_column(column=column, replacement='', to_be_replaced=element)
            self.df_cleared[column] = self.df_cleared[column].apply(lambda x: max(x.split('=')) if type(x) == str else x)
